...
export default function Magaza() {
  ...
  return (
    <YPLayout activeLink="/yourpoodle/magaza">
      ...
      {/* ── Footer ────────────────────────────────────── */}
      <footer style={{ background:"#4C1D95", padding:"40px 20px 24px" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          <div style={{ display:"grid", gridTemplateColumns:"2fr repeat(4,1fr) 1.5fr",
            ...
            <div>
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12 }}>
                ...
                <div>
                  <div style={{ fontSize:15, fontWeight:800, color:"#fff" }}>YourPoodle</div>
                  <div style={{ fontSize:10.5, color:"#C4B5FD" }}>Poodle'ınız için en iyi rehber</div>
                </div>
              </div>
              <p style={{ fontSize:12.5, color:"#A78BFA", lineHeight:1.65, marginBottom:16 }}>
                Poodle sahiplerine cost-benefit öneriler, mama önerileri ve sağlıklı yaşam için güvenilir platformunuz.
              </p>
              <div style={{ display:"flex", gap:12 }}>
                {["📸","📘","▶️","🎵"].map((ic,i)=>(
                  <div key={i} style={{ width:34, height:34, borderRadius:10,
                                        background:"rgba(255,255,255,0.12)", display:"flex",
                                        alignItems:"center", justifyContent:"center",
                                        fontSize:16, cursor:"pointer" }}>{ic}</div>
                ))}
              </div>
            </div>
            ...
            {/* Kurumsal */}
            <div>
              <div style={{ fontSize:12.5, fontWeight:800, color:"#fff", letterSpacing:"0.05em",
                            textTransform:"uppercase", marginBottom:14 }}>Kurumsal</div>
              {["Hakkımızda","Kariyer","Basın","Gizlilik Politikası","Kullanım Şartları"].map(l=>(
            ...
            <div>
              <div style={{ fontSize:12.5, fontWeight:800, color:"#fff", letterSpacing:"0.05em",
                            textTransform:"uppercase", marginBottom:14 }}>Mağaza</div>
              {["Tüm Ürünler","Poodle Mamaları","Bakım Ürünleri","Aksesuarlar","İndirimli Ürünler"].map(l=>(
                <a key={l} href="#" className="ft-link">{l}</a>
              ))}
            </div>
            ...
            <div>
              <div style={{ fontSize:12.5, fontWeight:800, color:"#fff", letterSpacing:"0.05em",
                            textTransform:"uppercase", marginBottom:14 }}>Destek</div>
              {["Sık Sorulan Sorular","Kargo ve Teslimat","İade ve Değişim","Ödeme Seçenekleri","İletişim"].map(l=>(
                <a key={l} href="#" className="ft-link">{l}</a>
              ))}
            </div>
            ...
            <div>
              <div style={{ fontSize:12.5, fontWeight:800, color:"#fff", letterSpacing:"0.05em",
                            textTransform:"uppercase", marginBottom:14 }}>Topluluk</div>
              {["Poodle Rehberi","Poodle Araçları","Topluluk","Etkinlikler","Bize Ulaşın"].map(l=>(
                <a key={l} href="#" className="ft-link">{l}</a>
              ))}
            </div>
            ...
            <div>
              <div style={{ fontSize:12.5, fontWeight:800, color:"#fff", letterSpacing:"0.05em",
                            textTransform:"uppercase", marginBottom:14 }}>İletişim</div>
              {[
                { icon:"📞", text:"+90 555 123 45 67" },
                { icon:"✉️", text:"info@yourpoodle.com" },
                { icon:"🕐", text:"Hafta içi 09:00 – 18:00" },
              ].map(({ icon, text })=>(
                <div key={text} style={{ display:"flex", alignItems:"flex-start", gap:8, marginBottom:10 }}>
                  <span style={{ fontSize:13 }}>{icon}</span>
                  <span style={{ fontSize:12.5, color:"#C4B5FD", lineHeight:1.4 }}>{text}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ borderTop:"1px solid rgba(255,255,255,0.12)", paddingTop:20,
            ...
            <span style={{ fontSize:12.5, color:"#A78BFA" }}>© 2026 YourPoodle. Tüm hakları saklıdır.</span>
            {/* Payment logos */}
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              {["VISA","MC","TROY","iyzico","SSL"].map(l=>(
                <div key={l} style={{ background:"rgba(255,255,255,0.15)", borderRadius:6,
