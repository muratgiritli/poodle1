        ...
        <header style={{ position:"sticky",top:0,zIndex:100,background:"#fff",borderBottom:"1px solid #f0f0f0" }}>
          <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 16px" }}>
            ...
            <div style={{ display:"flex",alignItems:"center",gap:4 }}>
              <button className="icon-btn" style={{ position:"relative" }}>
                ...
                {cart.length > 0 && <div style={{ position:"absolute",top:2,right:2,width:14,height:14,borderRadius:"50%",background:"#7C3AFF",display:"flex",alignItems:"center",justifyContent:"center" }}><span style={{ fontSize:8,fontWeight:900,color:"#fff" }}>{cart.length}</span></div>}
              </button>
          ...
          <div style={{ display:"flex",borderTop:"1px solid #f0f0f0" }}>
            {[{label:"Ana Sayfa",href:"/"},{label:"Rehber",href:"/rehber"},{label:"Bilgi Bankası",href:"/bilgi"},{label:"Mağaza",href:"/magaza"}].map(t=>(
              <a key={t.label} href={t.href} style={{ flex:1,padding:"11px 4px",fontSize:13.5,fontWeight:t.label==="Mağaza"?700:500,color:t.label==="Mağaza"?"#6C47FF":"#555",borderBottom:t.label==="Mağaza"?"2.5px solid #6C47FF":"2.5px solid transparent",textAlign:"center",textDecoration:"none",display:"block" }}>{t.label}</a>
            ))}
          </div>
        </header>
