...
const CSS = [
  ...
  ".yp-input:focus { border-color: #7C3AFF !important; box-shadow: 0 0 0 3px rgba(124,58,255,0.12); outline: none; }",
      ...
      {labels.map((s, i) => (
        <div key={s} style={{ display:"flex", alignItems:"center", flex:1 }}>
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", flex:1 }}>
            <div style={{ width:28, height:28, borderRadius:"50%", background: i<=idx ? "#7C3AFF" : "#f0f0f0", color: i<=idx ? "#fff" : "#bbb", display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:800, fontFamily:"Inter,sans-serif", transition:"all 0.2s" }}>
              {i < idx ? "✓" : i+1}
            </div>
            <span style={{ fontSize:10, fontWeight:600, color: i<=idx ? "#7C3AFF" : "#bbb", marginTop:4, fontFamily:"Inter,sans-serif" }}>{s}</span>
          </div>
          {i < labels.length-1 && <div style={{ height:2, flex:1, background: i<idx ? "#7C3AFF" : "#f0f0f0", marginBottom:16, transition:"background 0.3s" }} />}
        </div>
      ))}
    </div>
  );
...
function CountryPicker({ selected, onSelect, onClose }: {
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:300, display:"flex", flexDirection:"column", background:"rgba(0,0,0,0.4)" }}
      onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={{ marginTop:"auto", background:"#fff", borderRadius:"20px 20px 0 0", maxHeight:"80vh", display:"flex", flexDirection:"column" }}>
        {/* Header */}
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"16px 18px 12px", borderBottom:"1px solid #f0f0f0" }}>
          <span style={{ fontSize:16, fontWeight:800, color:"#1a1a1a", fontFamily:"Inter,sans-serif" }}>Ülke Kodu Seç</span>
...
/* ─── Social button ─────────────────────────────────────── */
function SocialBtn({ logo, label, color, bg, border, onClick }: {
  logo: React.ReactNode; label: string; color: string; bg: string; border: string; onClick: () => void;
}) {
  return (
    <button onClick={onClick} className="yp-btn"
      style={{ height:50, borderRadius:14, background:bg, border:`1.5px solid ${border}`, color, fontSize:14, display:"flex", alignItems:"center", justifyContent:"center", gap:10, width:"100%" }}>
      {logo}
...
function Toast({ msg, onDone }: { msg: string; onDone: () => void }) {
  ...
  return (
    <div style={{ position:"fixed", bottom:90, left:"50%", transform:"translateX(-50%)", background:"#1a1a1a", color:"#fff", padding:"10px 20px", borderRadius:12, fontSize:13, fontWeight:600, fontFamily:"Inter,sans-serif", zIndex:500, whiteSpace:"nowrap", boxShadow:"0 4px 20px rgba(0,0,0,0.3)" }}>
      {msg}
    </div>
  );
}
...
export default function YPGiris() {
  ...
  const sendOtp = async () => {
    ...
    const normalized = country.code === "TR"
      ? local
      : `${country.dial.replace("+","")}${local}`;
  ...
  /* ── Social handlers ── */
  const handleGoogle = () => setToast("Google girişi yakında aktif olacak 🚀");
      ...
      <div style={{ minHeight:"100vh", background:"#fff", fontFamily:"Inter,sans-serif", display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ background:"linear-gradient(135deg,#7C3AFF,#A855F7)", padding:"28px 24px 36px", position:"relative", overflow:"hidden" }}>
          ...
          <h1 style={{ fontSize:22, fontWeight:900, color:"#fff", marginBottom:4 }}>
            {step==="phone" ? "Hoş Geldiniz!" : step==="otp" ? "Kodu Girin" : "Profilinizi Tamamlayın"}
          </h1>
          <p style={{ fontSize:13, color:"rgba(255,255,255,0.82)", lineHeight:1.5 }}>
            {step==="phone" ? "YourPoodle'a giriş yapın veya üye olun." :
             step==="otp"   ? `${country.dial} ${phone} numarasına gönderilen 4 haneli kodu girin.` :
        ...
        <div style={{ flex:1, padding:"28px 20px 40px", background:"#fff", borderRadius:"20px 20px 0 0", marginTop:-12, boxShadow:"0 -4px 20px rgba(0,0,0,0.06)" }}>
          ...
          {step==="phone" && (
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              ...
              {/* Social buttons */}
              <SocialBtn logo={<GoogleLogo />} label="Google ile Devam Et"
                color="#333" bg="#fff" border="#e0e0e0" onClick={handleGoogle} />
              <SocialBtn logo={<FacebookLogo />} label="Facebook ile Devam Et"
                color="#fff" bg="#1877F2" border="#1877F2" onClick={handleFacebook} />
              ...
              <div>
                ...
                <div style={{ display:"flex", border:"2px solid #e8e8e8", borderRadius:14, overflow:"hidden", background:"#fafafa" }}>
                ...
                <div style={{ display:"flex", gap:12, justifyContent:"center", marginBottom:6 }}>
                  {otp.map((d,i) => (
                    <input key={i} ref={el => otpRefs.current[i]=el}
                      ...
                      style={{ width:62, height:72, borderRadius:16, textAlign:"center", fontSize:28, fontWeight:900, fontFamily:"Inter,sans-serif", border:`2.5px solid ${d?"#7C3AFF":"#e8e8e8"}`, background:d?"#F5F0FF":"#fafafa", color:"#7C3AFF", outline:"none", transition:"all 0.15s", caretColor:"#7C3AFF" }} />
                  ))}
                </div>
                {error && <p style={{ fontSize:12, color:"#EF4444", fontWeight:600, marginTop:4 }}>{error}</p>}
              </div>
          ...
          {step==="register" && (
            <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
              ...
              <div>
                ...
                <input className="yp-input" type="text" value={name}
                  onChange={e => setName(e.target.value)}
                  onKeyDown={e => { if (e.key==="Enter") doRegister(); }}
                  placeholder="Örn: Ayşe" autoFocus
                  style={{ width:"100%", height:52, borderRadius:14, border:"2px solid #e8e8e8", padding:"0 16px", fontSize:16, fontWeight:600, color:"#222", background:"#fafafa", fontFamily:"Inter,sans-serif", transition:"border-color 0.2s" }} />
                {error && <p style={{ fontSize:12, color:"#EF4444", marginTop:6, fontWeight:600 }}>{error}</p>}
              </div>
              <div>
