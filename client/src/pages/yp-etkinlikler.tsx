...
export default function Etkinlikler() {
  ...
  const types = ["Tümü", ...Array.from(new Set(events.map((e:any) => e.type).filter(Boolean)))];
  const filtered = filter === "Tümü" ? events : events.filter((e:any) => e.type === filter);

  function handleCardClick(ev: any) {
    if (ev.slug) {
      navigate(`${BASE}/etkinlikler/${ev.slug}`);
    } else {
      setSelectedEvent(ev);
    }
  }
  ...
  return (
    <YPLayout activeLink="/yourpoodle/etkinlikler">
      ...
      <div style={{ minHeight:"100vh", background:"#fff", fontFamily:"Inter,sans-serif", paddingBottom:32 }}>
          ...
          {events.length > 0 && (
            <div style={{ display:"flex", gap:16, marginTop:16 }}>
              <div style={{ textAlign:"center" }}>
                <div style={{ fontSize:18, fontWeight:900, color:"#fff" }}>{events.length}</div>
                <div style={{ fontSize:10, color:"rgba(255,255,255,0.7)" }}>Etkinlik</div>
              </div>
              {(() => {
                const cities = new Set(events.map((e:any) => e.location).filter((l:string) => l && l !== "Online" && l !== "Zoom" && !l.toLowerCase().includes("online") && !l.toLowerCase().includes("zoom")));
                const online = events.filter((e:any) => e.type === "Online").length;
                return (
                  <>
                    {cities.size > 0 && (
                      <div style={{ textAlign:"center" }}>
                        <div style={{ fontSize:18, fontWeight:900, color:"#fff" }}>{cities.size}</div>
                        <div style={{ fontSize:10, color:"rgba(255,255,255,0.7)" }}>Şehir</div>
                      </div>
                    )}
                    {online > 0 && (
                      <div style={{ textAlign:"center" }}>
                        <div style={{ fontSize:18, fontWeight:900, color:"#fff" }}>{online}</div>
        ...
        {/* Loading state */}
        {isLoading && (
          <div style={{ padding:"48px 16px", textAlign:"center", color:"#888", fontSize:14 }}>
        ...
        {!isLoading && events.length === 0 && (
          <div style={{ padding:"48px 20px", textAlign:"center" }}>
            ...
            <div style={{ fontSize:16, fontWeight:800, color:"#1a1a1a", marginBottom:8 }}>Yaklaşan etkinlik yok</div>
            <p style={{ fontSize:13, color:"#888", lineHeight:1.6, maxWidth:280, margin:"0 auto" }}>
              ...
              <div key={ev.id||ev.title} className="ev-card" onClick={() => handleCardClick(ev)} style={{ background:"#fff", borderRadius:18, boxShadow:"0 2px 16px rgba(0,0,0,0.07)", overflow:"hidden", cursor:"pointer" }}>
                <div style={{ display:"flex", alignItems:"stretch" }}>
                  ...
                  <div style={{ flex:1, padding:"14px 14px 14px 12px" }}>
                    ...
                    {ev.description && <div style={{ fontSize:12, color:"#666", lineHeight:1.55, marginBottom:6, fontFamily:"Inter,sans-serif" }}>{ev.description}</div>}
                    <div style={{ display:"flex", alignItems:"center", gap:4 }}>
                      <MapPin size={11} color="#888" /><span style={{ fontSize:11, color:"#888", fontFamily:"Inter,sans-serif" }}>{ev.location}</span>
                    </div>
                  </div>
                </div>
                {!isLoggedIn && (
        ...
        {!isLoading && events.length > 0 && filtered.length === 0 && (
          <div style={{ padding:"32px 20px", textAlign:"center", color:"#888", fontSize:14 }}>
            Bu türde etkinlik bulunamadı.
          </div>
        )}

        {/* Event detail bottom sheet (for events without a slug) */}
        {selectedEvent && (
          <div style={{ position:"fixed", inset:0, zIndex:500, display:"flex", flexDirection:"column", justifyContent:"flex-end" }}
            ...
            <div onClick={e => e.stopPropagation()}
              ...
              {selectedEvent.description && (
                <p style={{ fontSize:14, color:"#555", lineHeight:1.7, marginBottom:16, fontFamily:"Inter,sans-serif" }}>{selectedEvent.description}</p>
              )}
              {/* Details */}
