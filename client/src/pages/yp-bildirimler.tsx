...
function notifIcon(type: string) {
  switch (type) {
    ...
    case "comment":      return <MessageCircle size={16} color="#5D3A1A" />;
    case "follow":
...
function notifColor(type: string) {
  switch (type) {
    ...
    case "comment":        return { bg: "#F5F0E6", border: "#E5DDD0" };
    case "follow":
    ...
    return (
      <YPLayout activeLink="/yourpoodle/bildirimler">
        <div style={{ minHeight: "60vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32, fontFamily: "Inter,sans-serif" }}>
          <Bell size={52} color="#D4C4B0" style={{ marginBottom: 16 }} />
      ...
      <div style={{ maxWidth: 600, margin: "0 auto", fontFamily: "Inter,sans-serif", minHeight: "100vh", background: "#FAFAFA" }}>
        ...
        <div style={{ background: "#fff", borderBottom: "1px solid #F0EAFF", padding: "12px 16px", display: "flex", alignItems: "center", gap: 10, position: "sticky", top: 60, zIndex: 40 }}>
          ...
          <h1 style={{ fontSize: 17, fontWeight: 900, color: "#1a1a1a", margin: 0, flex: 1 }}>
            Bildirimler {unread > 0 && (
              <span style={{ fontSize: 12, background: "#5D3A1A", color: "#fff", borderRadius: 10, padding: "2px 8px", marginLeft: 8, fontWeight: 800 }}>
                {unread}
              </span>
            )}
          </h1>
          {unread > 0 && (
            <button
              ...
              style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 12px", borderRadius: 10, border: "none", background: "#F5F0E6", color: "#5D3A1A", fontSize: 12, fontWeight: 800, cursor: "pointer" }}>
        ...
        {isLoading ? (
          <div style={{ display: "flex", justifyContent: "center", alignItems: "center", padding: 64 }}>
            <div style={{ width: 32, height: 32, borderRadius: "50%", border: "3px solid #D4C4B0", borderTopColor: "#5D3A1A", animation: "spin 0.8s linear infinite" }} />
            ...
            {notifs.map(n => {
              ...
              return (
                <div key={n.id}
                  ...
                  {!n.is_read && (
                    <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#5D3A1A", flexShrink: 0, marginTop: 4 }} />
                  )}
                </div>
              );
            })}
